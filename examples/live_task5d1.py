"""Task 5D.1 live proof: the prior-art challenge rerun through the
corrected query boundary.

Task 5D's live evidence isolated unintended conjunctivity: ten-plus-term
model queries executed as all-term conjunctions and sixteen of eighteen
searches returned nothing. The model now proposes concept groups and the
trusted boolean-v1 renderer builds explicit Boolean expressions; this
run challenges the same three preserved candidates through that
boundary, under the same directive bounds, thresholds, and cutoff as
Task 5D — frozen before the run, compared against the recorded Task 5D
baseline after it, and never tuned to help a candidate survive.
...
corpus. The preserved Task 5D run is untouched::
...
#: The Task 5D live evidence this rerun is measured against —
#: diagnostic, never an acceptance target.
TASK5D_BASELINE = {
    "searches": 18,
    "zero_result_searches": 16,
    "unique_pools": "2-4",
    "credits": 180,
    "model_calls": 9,
    "repairs": 0,
...
def main() -> int:
    ...
    print("== Task 5D.1 live proof: the challenge through the "
          "corrected query boundary ==")
...
def _report(
    ...
    print("-- against the Task 5D baseline (diagnostic, not a target) --")
    print(
        f"searches   : {len(result.executions)} now vs "
        f"{TASK5D_BASELINE['searches']} then; zero-result "
        f"{zero_result} now vs "
        f"{TASK5D_BASELINE['zero_result_searches']} then"
    )
    pools = ", ".join(
    ...
    print(
        ...
        f"{TASK5D_BASELINE['unique_pools']} then"
    )
    print()
